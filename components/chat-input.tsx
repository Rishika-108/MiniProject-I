...
//             disabled={disabled}
//             className={`absolute right-2 top-1/2 -translate-y-1/2 h-8 w-8 p-0 transition-all duration-300 ${isListening
...
export function ChatInput({ onMessage, disabled = false }: ChatInputProps) {
  ...
  const sendMessage = async () => {
    ...
    // Only send USER message upward
    onMessage({ role: "user", text: userText });

    setMessage("");
    inputRef.current?.focus();

    // ❗ ChatInput DOES NOT handle AI messages anymore
  };
